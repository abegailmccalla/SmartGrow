...
class DB:
    ...
    def addUpdate(self,data):
        ...
        try:
            ...
            result      = remotedb.COMP3901.greenhouse.insert_one(data)
    ...
    def getAllInRange(self,start, end):
        ...
        try:
            ...
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.COMP3901.greenhouse.find({"timestamp": {'$gte': start, '$lte': end}},{'_id':0}).sort('timestamp', 1))
        except Exception as e:
            msg = str(e)
    ...
    def humidityMMAR(self,start, end):
        ...
        try:
            ...
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.COMP3901.greenhouse.aggregate( [
            {
                '$match': {
                    'timestamp': {
                        '$gte': start, 
    ...
    def temperatureMMAR(self,start, end):
        ...
        try:
            ...
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result = list(remotedb.COMP3901.greenhouse.aggregate([{ '$match': { 'timestamp': { '$gte': start, '$lte': end } } }, { '$group': { '_id': 0, 'temperature': { '$push': '$$ROOT.temperature' } } }, { '$project': { 'max': { '$max': '$temperature' }, 'min': { '$min': '$temperature' }, 'avg': { '$avg': '$temperature' }, 'range': { '$subtract': [ { '$max': '$temperature' }, { '$min': '$temperature' } ] } } } ]))
